...
با تغییر داده ساختار semaphore به این صورت که لیست ریسه های منتظر آن را ( waiters) به یک Max_Heap_queue تبدیل می کنیم ( یعنی waiter_pq)  تا در هنگام sema_up ریسه منتظر با اولویت موثر بیشینه آنبلاک کنیم. 
...
قبل از آنکه ریسه ی جاری برای گرفتن قفل به حالت انتظار برود ( sema_down (&lock->semaphore)) به دارنده ی فعلی قفل  (lock -> holder ) effective_priority برابر با effective_priority ریسه جاری نسبت می دهیم (فقط در جهت افزایش). فرض کنید ابتدا ریسه B به A اهدا میکند. سپس اگر A بخواهد منتظر قفلی در اختیار  C شود، چون effective_priority خود را به C اهدا میکند، پس C نیز اولویت موثری برابر B را خواهد گرفت.
...
همچنین lock_release از لیست اولویت دار مربوط به سمافور قفل (waiters_pq)، ریسه با بیشترین اولویت را pop کرده و در ready_list قرار میدهد. و اگر پس از push کردن در ready_list (که خود یک لیست اولویت دار است) اولویت ریشه آن بیشتر از ریسه فعلی بود، نشان میدهد که ریسه با اولویت بالاتری در سیستم آماده اجراست و سریعاً باید thread_yield() را صدا بزنیم. در غیر این صورت، به اجرا ادامه میدهیم.
### همگام‌سازی
